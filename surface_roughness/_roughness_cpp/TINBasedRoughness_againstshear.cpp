...
std::vector<double> TINBasedRoughness_againstshear::get_points()
{
    arma::rowvec pout = points.as_row();
    return arma::conv_to<std::vector<double>>::from(pout);
}

std::vector<double> TINBasedRoughness_againstshear::get_normals()
{
    arma::rowvec nout = normals.as_row();
    return arma::conv_to<std::vector<double>>::from(nout);
}

arma::vec TINBasedRoughness_againstshear::plane_fit(const arma::mat& xyz) {
    using namespace arma;
...
arma::vec TINBasedRoughness_againstshear::plane_normal(const arma::mat& xyz) {
    using namespace arma;
...
void TINBasedRoughness_againstshear::calculateNormals()
{
    using namespace arma;
...
void TINBasedRoughness_againstshear::calculateAreas()
{
    using namespace arma;
...
std::vector<double> get_area_vector(
	std::vector<TIN_triangle_againstshear> evaluation, 
	std::function<double(TIN_triangle_againstshear)> area_op)
{
...
void TINBasedRoughness_againstshear::evaluate(TINBasedRoughness_settings settings, bool verbose_,std::string file_path)
{
    ...
    this->calculateNormals();
    this->calculateAreas();
	typedef std::vector<TIN_triangle_againstshear> TriangleContainer;
	...
	delta_n_.zeros(n_directions);
    delta_star_n_.zeros(n_directions);
	...
	[&](TIN_triangle_againstshear& triangle) {
        triangle.index = counter++;
        triangle.area = *area_iter++;	
        triangle.set_normal(*normal_x_iter++, *normal_y_iter++, *normal_z_iter++);
        triangle.v1v0 = this->points.row(*tri_1_iter) - this->points.row(*tri_0_iter);
        triangle.v2v0 = this->points.row(*tri_2_iter) - this->points.row(*tri_0_iter);
        tri_0_iter++;
        tri_1_iter++;
        tri_2_iter++;
	});
    ...
    if (this->triangles.n_rows < settings_["min_triangles"]) {
		...
		this->parameters.insert({"delta_n",conv_to<stdvec>::from(delta_n_)});
		this->parameters.insert({"delta*_n", conv_to<stdvec>::from(delta_star_n_)});
		return;
	}
	...
	for (size_t az_i = 0; az_i < azimuths_.n_elem; ++az_i) {
		...
		std::for_each(evaluation.begin(), evaluation.end(),
			[&az_i, &cartesian_az](TIN_triangle_againstshear& triangle) {
		...
		auto remove_it = std::remove_if(evaluation.begin(), evaluation.end(),
			[&](const TIN_triangle_againstshear& triangle) {
...
arma::mat TINBasedRoughness_againstshear::pol2cart(arma::vec azimuth) 
{
	using namespace arma;
