...
struct MeanDip_triangle
{
	...
	*/
	...
	MeanDip_triangle(int index, arma::vec normal) :
		index(index),apparent_dip_angle(0)
	{
		normal_x = normal(0);
		normal_y = normal(1);
		normal_z = normal(2);
...
{
public:
	MeanDipRoughness(const std::vector<double>& points, const std::vector<uint64_t>& triangles);
	MeanDipRoughness(const std::vector<double>& points, const std::vector<uint64_t>& triangles, const std::vector<uint64_t>& selected_triangles);
